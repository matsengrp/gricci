# SAGE functions for computing the Ollivier-Ricci curvature by solving the integer linear problem.
#
...
def ollivier(g, source=0, target=1, t1=1, t2=4):
    ...
    p.set_objective(-p.sum(D[i,j]*x[i,j] for i in range(N) for j in range(N)))
